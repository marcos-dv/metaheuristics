...
public class Geometry {
	...
	public static double[] sum(double[] x, double[] y) {
		...
		if (x.length != y.length) {
			Messages.error(
					"Geometry: sum x and y dont have the same dimension (" + x.length + " and " + y.length + ")");
			return sum;
		}
		for (int i = 0; i < x.length; ++i) {
			sum[i] = x[i] + y[i];
	...
	public static double[] diff(double[] x, double[] y) {
		...
		if (x.length != y.length) {
			Messages.error(
					"Geometry: diff x and y dont have the same dimension (" + x.length + " and " + y.length + ")");
			return sum;
		}
		for (int i = 0; i < x.length; ++i) {
			sum[i] = x[i] - y[i];
	...
	public static double dist(double[] x, double[] y) {
		if (x.length != y.length) {
			Messages.error(
					"Geometry: dist x and y dont have the same dimension (" + x.length + " and " + y.length + ")");
			return 0;
		}
	...
	public static double innerProduct(double[] x, double[] y) {
		...
		if (x.length != y.length) {
			Messages.error(
					"Geometry: innerProd x and y dont have the same dimension (" + x.length + " and " + y.length + ")");
			return 0;
		}
		for (int i = 0; i < x.length; ++i) {
			prod += x[i] * y[i];
	...
	public static double angle(double[] x, double[] y) {
		if (x.length != y.length) {
			Messages.error(
					"Geometry: angle x and y dont have the same dimension (" + x.length + " and " + y.length + ")");
			return 0;
		}
		double normx = norm(x);
		double normy = norm(y);
		if (normx == 0 || normy == 0) {
			Messages.error("Geometry: angle x or y have norm 0 (" + normx + " and " + normy + ")");
			return 0;
		}
	...
	public static double distPoint2Segment(double[] p, double[] q, double[] x) {
		if (p.length != q.length || x.length != q.length) {
			Messages.error("Geometry: distPoint2Segment p or q or x dont have the same dimension (" + p.length + " , "
					+ q.length + " , " + x.length + ")");
			return 0;
		}
		double[] pq = diff(q, p);
	...
	public static double distPoint2Line2D(double[] p, double[] q, double[] x) {
		if (p.length != q.length || x.length != q.length || x.length != 2) {
			Messages.error("Geometry: distPoint2Segment p or q or x dont have dimension 2 (" + p.length + " , "
					+ q.length + " , " + x.length + ")");
			return 0;
		}
