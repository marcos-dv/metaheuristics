...
public class ContinuousGame extends SimState implements Steppable {
	...
	public ContinuousGame(long seed, int w, int h, double discretization) {
		super(seed);
		width = w;
		height = h;
	...
	public ContinuousGame(long seed) {
		super(seed);
	}

	private void seedGrid() {
		for(int i = 0; i < numParticles; ++i) {
			double x = randomGen.nextDouble()*width;
			double y = randomGen.nextDouble()*height;
	...
	public void step(SimState arg0) {
		...
		for(int i = 0; i < numParticles; ++i) {
			...
			double newX = p.x + randomGen.nextDouble()-0.5;
			double newY = p.y + randomGen.nextDouble()-0.5;
