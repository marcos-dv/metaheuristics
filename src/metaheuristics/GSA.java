...
public class GSA implements IMetaheuristic {
	...
	protected void computeFitness() {
		...
		/*
		 * Multiple sols Cec2015Problem p = (Cec2015Problem) targetProblem; try { fit =
		 * p.contestFitnessMultipleSols(getSols()); } catch (Exception e) { // TODO
	...
	protected void processFitness() {
		...
		if (DEBUG) {
			...
			// Print best K
			for (int i = 0; i < sols.length; ++i) {
				System.out.println(i + ". " + kbest[i].index + " val = " + kbest[i].value);
			}
		}
	...
	protected double[][] calcDistances() {
		...
		for (int i = 0; i < sols.length; ++i) {
			for (int j = 0; j < kSize; ++j) {
				// Dist
				distances[i][j] = Algorithms.d2(sols[i].getCoords(), sols[kbest[j].index].getCoords());
				if (DEBUG)
		...
		for (int i = 0; i < sols.length; ++i) {
			...
			for (int d = 0; d < targetProblem.getDim(); ++d) {
				...
				for (int j = 0; j < kSize; ++j) {
					...
					if (DEBUG)
						System.out.println("x[" + i + "][" + d + "]-x[" + kbest[j].index + "][" + d + "] = " + xdiff);
					// acceleration update
	...
	public void updateGlobalBest() {
		...
		if ((globalBest == null) || (curIterationBest.getFitness() < globalBest.getFitness())) {
